...
1. **Add the expert's photo** to both:
   ...
   Both paths must have the same file. The image should be a square headshot (PNG format).
...
```tsx
{
  ...
  description: (
    <div>
      A short description of the expert's work and specialties. Links to projects can be included with <a> tags.
    </div>
  ),
},
```
